...
El procés d’\textbf{aprendre fent} ha estat essencial: he hagut de resoldre errors, buscar documentació, provar diferents solucions i finalment aconseguir un resultat.
Aquest aprenentatge és una de les parts més valuoses del treball.

\section{Limitacions}
\label{s:recomanacions}
...
\begin{itemize}
    \item \textbf{Prova d’integració:} establir un punt d’intercanvi senyalitzat a la Riera Blanca, o quan dues xarxes ja tenen estacions molt properes.
    \item \textbf{API unificada:} promoure la creació d’un portal conjunt de dades obertes que faciliti la unió entre Bicing i AMBici.
...
En el camp de la recerca, el final d'un treball pot ser l'inici d'un nou projecte de recerca. Per això crec convenient fer propostes de com es podria continuar la meva recerca.

\begin{enumerate}
 \item Desenvolupar una aplicació mòbil. Aquesta app permetria tenir tot el sistema al teu abast: consultar estacions pròximes, veure si hi ha bicicletes o espais disponibles, detectar punts de transbord i rebre recomanacions en temps real. Seria una manera de fer que el projecte fos encara més accessible i útil per a l’usuari final.
 \item Eliminar algunes de les limitacions comentades a~\ref{s:recomanacions} per portar més enllâ el projecte actual.
 \item Cercar altres conjunts de dades obertes per fer un projecte prenent aquest com a base.
\end{enumerate}
